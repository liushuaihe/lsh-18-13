...
import {
  ...
  AlertTriangle, Clock, Activity, Server, Package,
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';

export const TopBar: React.FC = () => {
  const navigate = useNavigate();
  const { devices, sensors, globalFuseActive, alerts, batches } = useMonitorStore();
  ...
  return (
    ...
    `}>
      <div className="flex items-center gap-3">
        ...
        <div className="w-px h-6 bg-gradient-to-b from-transparent via-cyber-cyan/30 to-transparent mx-2" />

        <button
          onClick={() => navigate('/batch-trace')}
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border border-cyber-cyan/40
            bg-cyber-cyan/5 text-cyber-cyan hover:bg-cyber-cyan/15 transition-colors text-xs"
        >
          <Package size={13} />
          <span className="font-medium">批次追溯</span>
          {batches.filter(b => b.status === 'pending').length > 0 && (
            <span className="ml-0.5 px-1.5 py-0.5 rounded-full bg-amber-500/20 text-amber-400 text-[9px] font-mono">
